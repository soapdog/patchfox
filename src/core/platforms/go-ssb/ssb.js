...
class GoSSB {
  ...
  aboutMessages(sourceId, destId) {
    return new Promise((resolve, reject) => {
      ...
      pull(
        sbot.partialReplication.getMessagesOfType(opts),
  ...
  async avatar(feed) {
    const getAvatarAux = async (feed) => {
      ...
      let name = await sbot.names.getSignifier({id: feed})
      let image = await sbot.names.getImageFor({id: feed})
